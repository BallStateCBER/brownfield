...
//@{
	 ...
	 * @since 0.4
	 */
	public function setLabelFormat($params = array())
	{
		if ( $this->chxs < 1 ) {
			$this->chxs = 1;
		}
		...
		$default_params = array(
			...
			'num_type' => 'f',
			'decimal_places' => '0',
			'trailing_zeros' => '', 		//blank or z
			'groups_separator' => 's', 		//blank or s
			'xy' => 'y',
			'following' => ''
		);
		$params = array_merge($default_params, $params);
		extract($params);
		$this->format = "N{$preceding}*{$num_type}{$decimal_places}{$trailing_zeros}{$groups_separator}{$xy}*{$following}";
