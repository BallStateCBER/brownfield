	...
	if (county_id) {
		...
		if (industry_index != 0) {
			...
			if (input_option_index != 0) {
				...
				(input_option_index == 1) ? user_input = $('calc_annual_production').getValue() : user_input = $('calc_employees').getValue();
	...
	new Ajax.Request(url, {
		...
		onSuccess: function(transport) {
			if (transport.responseText.match('Error')) {
				...
				industry_select.childElements().each(function(option) {
					option.show();
				});
			} else {
...
function calculateImpact(animate) {
	var county_id = $('calc_county_id').getValue();
	var industry_id = $('calc_industry_id').getValue();
	var selected_option = $('calc_input_options').selectedIndex;
	if (! county_id || ! industry_id) {
		return;
	}
	if (selected_option == 1) {
		var annual_production = inputToInt($('calc_annual_production').getValue());
		...
		var employees = inputToInt($('calc_employees').getValue());
		if (! employees) {
	...
	if (animate && container.visible()) {
		$$('.calc_section').each(function(section){
			Effect.DropOut(section, {
				duration: 0.2,
				queue: {position: 'front', scope: 'calc_section_dropout'},
				afterFinish: function() {
					var queue = Effect.Queues.get('calc_section_dropout');
					if (queue.size() == 0) {
						container.hide();
